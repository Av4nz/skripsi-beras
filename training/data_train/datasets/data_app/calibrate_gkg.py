...
def fill_missing_gkg(harga_beras_file, calibrated_file, target_dates, output_file=None):
    """
    Fill missing month(s) in an already-built calibrated GKG file.

    The reference for the missing month is the BPS 'harga_gkg' column from the
    harga_beras dataset. Because that column is BPS data while the calibrated
    file is on the BAPANAS scale, the reference value is calibrated with the
    same Mean Bias Correction factor used for BPS 2018 (mean of BAPANAS/BPS
    over the 2019 overlap) and tagged 'BPS (Calibrated)'.

    Rows that are completely blank (no date) are dropped and replaced by the
    freshly computed value at its correct chronological position.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    data_dir = os.path.join(base_dir, 'datav2', 'gkg')

    # 1. Recompute the calibration factor from the 2019 overlap
    bapanas_df = load_bapanas_data(os.path.join(data_dir, 'bapanas_2019-2026.csv'))
    bps_2019_df = load_bps_data(os.path.join(data_dir, 'bps_2019.csv'), 2019)
    factor = calculate_calibration_factor(bps_2019_df, bapanas_df, year=2019)

    # 2. Load BPS reference (harga_gkg) from the harga_beras dataset
    hb = pd.read_csv(harga_beras_file)
    hb['tanggal'] = pd.to_datetime(hb['tanggal'])
    ref = hb.set_index('tanggal')['harga_gkg']

    # 3. Load the existing calibrated file and drop fully-blank rows
    cal = pd.read_csv(calibrated_file)
    cal['Date'] = pd.to_datetime(cal['Date'], errors='coerce')
    cal = cal.dropna(subset=['Date'])

    # 4. Calibrate and insert each target month
    for d in target_dates:
        dt = pd.to_datetime(d)
        if dt not in ref.index:
            print(f"Warning: no BPS harga_gkg reference for {dt.date()}, skipping.")
            continue
        bps_val = float(ref.loc[dt])
        calibrated_val = bps_val * factor
        cal = cal[cal['Date'] != dt]  # remove any stale row for this date
        new_row = pd.DataFrame({'Date': [dt], 'Harga': [calibrated_val], 'Source': ['BPS (Calibrated)']})
        cal = pd.concat([cal, new_row], ignore_index=True)
        print(f"Filled {dt.date()}: BPS {bps_val} * {factor:.6f} = {calibrated_val:.6f}")

    # 5. Sort chronologically and write out
    cal = cal.sort_values('Date').reset_index(drop=True)
    cal['Date'] = cal['Date'].dt.strftime('%Y-%m-%d')

    out = output_file or calibrated_file
    cal.to_csv(out, index=False)
    print(f"Filled calibrated data ({len(cal)} rows) saved to {out}")
    return cal
